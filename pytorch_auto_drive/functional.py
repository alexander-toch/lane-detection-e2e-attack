...
from .functional_pil import resize as F_pil_resize
from .functional_tensor import _get_image_size, adjust_lighting
...
def _get_image_size(img: Tensor) -> List[int]:
    ...
    return _get_image_size(img)


def _get_image_num_channels(img: Tensor) -> int:
    if isinstance(img, torch.Tensor):
        return _get_image_num_channels(img)
...
def resize(img: Tensor, size: List[int], interpolation: int = Image.BILINEAR) -> Tensor:
    ...
    if not isinstance(img, torch.Tensor):
        return F_pil_resize(img, size=size, interpolation=interpolation)

    return resize_tensor(img, size=size, interpolation=interpolation)


def resize_tensor(img: Tensor, size: List[int], interpolation: int = 2) -> Tensor:
    ...
    # make image NCHW
    # Support HW and CHW
    need_squeeze = 4 - img.ndim
    for _ in range(need_squeeze):
        img = img.unsqueeze(0)
...
def rotate(
    ...
    if not isinstance(img, torch.Tensor):
        return F_pil.rotate(
            img, angle=angle, resample=resample, expand=expand, center=center, fill=fill
        )
...
def affine(
    ...
    if len(shear) != 2:
        raise ValueError(
            "Shear should be a sequence containing two values. Got {}".format(shear)
        )
...
def erase(
    img: Tensor, i: int, j: int, h: int, w: int, v: Tensor, inplace: bool = False
) -> Tensor:
...
def gaussian_blur(
    ...
    if not isinstance(kernel_size, (int, list, tuple)):
        raise TypeError(
            "kernel_size should be int or a sequence of integers. Got {}".format(
                type(kernel_size)
            )
        )
    ...
    if len(kernel_size) != 2:
        raise ValueError(
            "If kernel_size is a sequence its length should be 2. Got {}".format(
                len(kernel_size)
            )
        )
    for ksize in kernel_size:
        if ksize % 2 == 0 or ksize < 0:
            raise ValueError(
                "kernel_size should have odd and positive integers. Got {}".format(
                    kernel_size
                )
            )
    ...
    if sigma is not None and not isinstance(sigma, (int, float, list, tuple)):
        raise TypeError(
            "sigma should be either float or sequence of floats. Got {}".format(
                type(sigma)
            )
        )
...
def adjust_lighting(
    img: Tensor, lighting_factor: Tensor, eigen_value: Tensor, eigen_vector: Tensor
) -> Tensor:
    ...
    return adjust_lighting(img, lighting_factor, eigen_value, eigen_vector)
